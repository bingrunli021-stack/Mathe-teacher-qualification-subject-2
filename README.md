...
线上验收通过：真实账号登录、第一知识点原页图片加载及页码弹窗、AI蓝色与原文深灰颜色分离、笔记写入并在第二页面读取、35秒服务器计时、分秒显示，以及切换页面后恢复未保存笔记。验收笔记的当前内容已恢复为空白，历史版本按设计保留。两页面属于同一浏览器，不能替代三台真实设备测试。

待办：第一章逐页OCR校对、精确知识点边界、结构化表格、补充自测题、电脑/手机/平板三设备验收。第一章验收前不批量导入其他章节正文。
